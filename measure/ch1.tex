...
\thmp{(\textbf{An equivalent formultion of measurability}) Let $E$ be measurable and $E_0 \subseteq E$. If $m^*(E)=m^*(E_0)+m^*(E-E_0)$ then $E_0$ is measurable and naturally, so is $E-E_0$. The converse statement is obviously true.}{Consider $E\subset O$ such that $m(O-E)\leq \varepsilon$. Let $O_1$ and $O_2 $ be so that $m^*(O_1)\leq m^*(E_0)+\varepsilon$ and $m^*(O_2)\leq m^*(E-E_O)+\varepsilon$. $E_0\subseteq O_1$, $E-E_O\subseteq O_2$. $$m^*(E_0)+m^*(E-E_0)=m^*(E)\leq m^*(O_1 \cup O_2)=m^*(O_1)+m^*(O_2)-m^*(O_1\cap O_2) $$ which yields $m^*(O_1 \cap O_2) \leq 2\varepsilon$. Note that $O_1-E_0\subseteq (O_1 \cap O_2) \cup ((O_1 \cup O_2)-E)$ which gives $$m^*(O_1-E_0)\leq m^*(O_1\cap O_2)+m^*(O-E)$$ which are each dominated by some factor of $\varepsilon$. Hence, $E_0$ is measurable. }
...
\thmp{Let $E\subseteq \RR^d$ and $O_n$ be the open set $$O_n=\{x\in\RR^d: d(x,E)<1/n\} $$ If $E$ were compact, then $$\lim_{n\to \infty} m(O_n)=m(E) $$ But there exists counter examples when:\begin{enumerate}
    \item $E$ is closed and unbounded
    \item $E$ is open and bounded
\end{enumerate} }{Note that $O_{n+1}\subseteq O_n \cdots $ a decreasing sequence. If $E$ were compact, all these sets have finite measure. So, $$m(\cap_n O_n)=\lim_{n\to\infty}m(O_n)$$ But note that $\cap_n O_n=\{x: d(x,E)=0\}$ which is $cls(E)$, which is in this case $E $ itself. Therefore, $$\lim_{n\to\infty}m(O_n)=m(E) $$
For the counter examples:
\begin{enumerate}
    \item Consider $\RR\subseteq \RR^2$, whose closure is itself. Note that $m(R)=0$ in $\RR^2$. But $O_n:=\{x:d(x,\RR)<1/n\}$ is an open set (its a strip) whose measure is $\infty$. This is true for all $O_n$, so $\lim_{n\to\infty}m(O_n)=\infty$ but $m(\RR)=0$.
    \item For the 2nd counter example: Let $\hat{C}$ be the fat cantor set, i.e, the cantor set created by removing intervals of size $l_0,l_1\cdots$ such that $1-(l_0+2l_1+2^2l_2 \cdots)>0$. Consider the union of all the deleted neighbourhoods. Note that, the fat cantor set is perfect, i.e, every point of $\hat{C}$ is a limit point of $\hat{C}$, so for $x\in\hat{C}$ there exists a monotonic sequence $x_n\in \hat{C}$ that converges to $x \in \hat{C}$. We also know $\hat{C}$ is totally disconnected, so between any two elements of $\hat{C}$, there is an element of $\hat{C}^c$, which is an element of the union of the deleted intervals. Hence, every element of $\hat{C}$ is a limit point of the union of the deleted neighbourhoods. Let $U$ denote this union of all deleted neighbourhoods at each step. We note that $cls(U)=[0,1]$. $m(cls(U))=m(\cap_n O_n)=1$, but $m(U)<1$ (because fat cantor set is of finite measure).  
\end{enumerate}}
\begin{example}
    (\textbf{There exists a sequence of decreasing, continuous functions whose pointwise limit is not Riemann integrable}) Let $\hat C$ denote the fat cantor set. Consider $F_0$ to be $1$ on all of $[0,1]$. Let $F_1$ to be $1$ on $C_1$ (2 intervals each of size $1-l_0/2$), $0$ at the centre of the deleted neighbourhood, and linear, connecting $0$ and $1$ on the deleted neighbourhood. Likewise, make $F_2$ so that it is $1$ on $C_2$, and $0$ on the centres of each deleted neighbourhoods, and is linear on these deleted neighbourhoods. We then define $f_n=F_1 \cdot F_2 \cdot F_n$, which is a sequence of decreasing functions. Note that, if $x \in \hat C$, there exists large enough $n$ such that $f_n=1$ on $x$. Therefore, on $\hat C$, the pointwise limit (which exists since each function is bounded) is $1$. We note that $f$ is discontinuous at every point of $\hat{C}$, since there exists a sequence $x_n$ outside $\hat{C}$ such that $f(x_n)\to 0$. Since the set of discontinuities is unconutable, the function $f$ is not riemann integrable (See appendix). 
\end{example}
\thmp{Let $f_n$ be a sequence of functions on $[0,1]$ such that $$|f(x)|<\infty \ a.e.$$ then there exists a sequence $c_n$ of real numbers such that $$f_n/c_n \to 0 \ a.e.$$}{Consider $$E_n^k:=\{x\in[0,1]: |f_n(x)|>k/n\} $$ For increasing $k$ this sequence of sets is decreasing, and $\cap_k E_n^k=\{x:|f_n(x)|=\infty\}$. Also, $E_n^k$ are measurable and of finite measure since they're subsets of $[0,1]$. Hence, $\lim_{k\to\infty}m(E^k_n)=m(\cap_k E^k_n)=0$. This means that, for a given $j$, there exists $n_0$ such that $$m(E^k_n)<2^{-j} \ \forall \ k \geq n_0$$ or simply, there exists, for every $j$, a $k_j$ such that $$m(E^{k_j}_j)=m(\{x\in[0,1]:|f_j(x)/k_j|>1/j\})<2^{-j}$$ Which means $\sum_j m(E^{k_j}_j)<\infty$, which means from Borel-Cantelli, $$m(\limsup_j E^{k_j}_j)=m(\cap_{n\in\NN}(\cup_{i\geq n}E^{k_i}_i ))=m(\cap_{n\in\NN}(\cup_{i\geq n}\{x\in[0,1]: |f_i(x)/k_i|>1/i\}))=0 $$ This means that the set $R=\cup_{n\in\NN}(\cap_{i\geq n} \{x: |f_i(x)/k_i|\leq 1/i\}) $ is a set of full measure, whose complement is null. If $x$ is in $R$, $|f_i(x)/k_i|\to 0$ eventually. Thus, a.e. $$f_i(x)/k_i\to 0 \ a.e. $$ }
\thmp{Every measurable function is an a.e. limit of a sequence of continuous functions.}{Let $f:\RR^d\to \RR\cup\{\pm \infty\}$ be a measurable function. Let $f_n$ be a sequence of step functions such that $$f_n\to f \ \text{pointwise }a.e. $$
Look at $\chi_{R}$ for a rectangle $R$. In $1-d$, it is easy to construct a sequence of continuous functions that converge to $\chi_I$; Let $R=\Pi_{i=1}^d[a_i,b_i]$. For the 1-d case, say on $[a_i,b_i]$, we can construct the function: $$\phi_{i,n}(x):=\begin{cases}
    0 \ :x \not \in [a_i-1/n,b_i+1/n] \\
    1 \ :x \in [a_i,b_i] \\
    n(x-a_i+1/n) \ :x \in [a_i-1/n,a_i]\\
    -n(x-b_i-1/n)\ :x \in [b_i,b_i+1/n]

\end{cases} $$ This function is continuous, and converges to $\chi_{[a_i,b_i]}$ pointwise, on the real line. Consider $f_n:\RR^d\to \RR\cup \{\pm \infty\}$ $f_n(x_1,x_2,\cdots x_d)=\Pi_{i=1}^d \phi_{i,n}(x_i)$. Does $f_n$ approximate $\chi_R$ well? Let $\vec{x}=(x_1,x_2,\cdots x_d) \in R$. That means $a_i\leq x_i\leq b_i$. Inside $R$, $\chi_R(x)=1$, which, of course, $f_n$ is. Outside $R$, $\chi_R$ is $0$. Let $\vec{x}=(x_1,x_2\cdots ,x_d) \in R^c$  }
\section{Integration}
...
\\\\ $\impliedby)$ Let $\varepsilon>0$, consider $\sup_{P} \U(P,f)=\inf_{P}\mathscr{L}(P,f)<\sup_{P}\U(P,f) +\varepsilon$. This means  there exists some $P_0$ such that $\mathscr{L}(P_0,f)\leq \sup_{P}\U(P,f)+\varepsilon$ which then again gives $P_1$ such that $\mathscr{L}(P_0,f)\leq \U(P_1,f)+\varepsilon$. Since $\mathscr{L}$ is a decreasing function for increasing partitions (more finer partitions) and $\mathscr{U}$ is decreasing for finer partitions, we combine $P_0$ and $P_1$ to yield a finer partition $P'''$ such that $\mathscr{L}(P''',f)-\mathscr{U}(P''',f)<\varepsilon$. We are done.
}
